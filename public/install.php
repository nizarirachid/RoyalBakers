...
function requirements(): array {
    ...
    foreach (['pdo', 'pdo_mysql', 'mbstring', 'openssl', 'tokenizer', 'xml', 'ctype', 'json', 'bcmath', 'fileinfo', 'gd', 'curl'] as $ext) {
        $list[] = ['n' => "امتداد PHP: $ext", 'ok' => extension_loaded($ext), 'h' => "تفعيل الامتداد $ext في php.ini"];
    }
    ...
    $list[] = ['n' => 'exec() مفعّل',        'ok' => execAvailable(),                             'h' => 'يجب تفعيل exec() في إعدادات PHP'];
...
if ($step === 'install' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // 1 — Write .env
    if (!writeEnv($p)) {
        $log[] = ['label' => 'كتابة ملف .env', 'ok' => false, 'out' => 'فشل الكتابة — تأكد أن المجلد الرئيسي قابل للكتابة (chmod 755)'];
        $step = 'done'; goto finish;
    }
    $log[] = ['label' => 'كتابة ملف .env', 'ok' => true, 'out' => 'تم إنشاء .env بنجاح'];

    // 2 — key:generate
    $r = runArtisan('key:generate --force');
    $log[] = ['label' => 'توليد مفتاح التطبيق (APP_KEY)', 'ok' => $r['ok'], 'out' => $r['out']];
    if (!$r['ok']) { $step = 'done'; goto finish; }

    // 3 — storage:link
    $r = runArtisan('storage:link --force');
    $log[] = ['label' => 'ربط مجلد storage', 'ok' => $r['ok'], 'out' => $r['out']];

    // 4 — migrate --seed
    $r = runArtisan('migrate --seed --force');
    $log[] = ['label' => 'إنشاء جداول قاعدة البيانات والبيانات الأولية', 'ok' => $r['ok'], 'out' => $r['out']];
    if (!$r['ok']) { $step = 'done'; goto finish; }

    // 5 — optimize
    $r = runArtisan('optimize');
    $log[] = ['label' => 'تحسين أداء التطبيق (cache)', 'ok' => $r['ok'], 'out' => $r['out']];

    // 6 — Self-delete
    @unlink(__FILE__);
    $log[] = ['label' => 'حذف ملف التنصيب (للأمان)', 'ok' => true, 'out' => 'تم حذف install.php تلقائياً'];
...
.logo h1{font-size:1.8rem;color:#f8c642;letter-spacing:.02em}
...
.ico.ok{background:#10b981;color:#fff}
.ico.fail{background:#ef4444;color:#fff}
...
.btn-primary{background:#f8c642;color:#0f172a}
...
.log-item .status.ok{background:#10b981;color:#fff}
.log-item .status.fail{background:#ef4444;color:#fff}
...
<div class="wrap">
  ...
  <div class="card">
    <h2>فحص المتطلبات</h2>
    <?php if (!$canProceed): ?>
    <div class="alert alert-warn">⚠️ بعض المتطلبات غير مستوفاة. يرجى إصلاحها قبل المتابعة.</div>
    <?php else: ?>
    <div class="alert alert-success">✅ جميع المتطلبات مستوفاة — يمكنك المتابعة.</div>
    <?php endif; ?>
    <ul class="req-list">
      ...
      <li class="req-item">
        ...
        <div>
          <div><?= htmlspecialchars($c['n']) ?></div>
          <?php if (!$c['ok']): ?><div class="hint"><?= htmlspecialchars($c['h']) ?></div><?php endif; ?>
        </div>
      </li>
      <?php endforeach; ?>
    </ul>
    <div class="btn-center">
      <?php if ($canProceed): ?>
      <a href="?step=config" class="btn btn-primary">التالي: ضبط الإعدادات &rarr;</a>
      <?php else: ?>
      <a href="?step=check" class="btn btn-secondary">إعادة الفحص</a>
      <?php endif; ?>
    </div>
  </div>
  ...
  <form method="POST" action="?step=install">
    <div class="card">
      ...
      <div class="row">
        ...
        <div class="field"><label>رابط الموقع (URL)</label><input name="APP_URL" value="<?= htmlspecialchars($D['APP_URL']) ?>"></div>
      </div>
      <div class="row">
    ...
    <div class="card">
      ...
      <div class="row">
        <div class="field"><label>كلمة السر</label><input name="DB_PASSWORD" type="password" value="<?= htmlspecialchars($D['DB_PASSWORD']) ?>"></div>
      </div>
    </div>
    <div class="card">
    ...
    <div class="card">
      <h2>حساب المدير الأعلى (Super Admin)</h2>
      <div class="row">
      ...
      <div class="row">
        <div class="field"><label>كلمة السر</label><input name="SUPER_ADMIN_PASSWORD" type="password" value="<?= htmlspecialchars($D['SUPER_ADMIN_PASSWORD']) ?>"><small>احتفظ بها في مكان آمن</small></div>
      </div>
    </div>
    <div class="btn-center">
      <a href="?step=check" class="btn btn-secondary" style="margin-left:1rem">السابق</a>
      <button type="submit" class="btn btn-primary">&#x26A1; بدء التنصيب التلقائي</button>
    </div>
  </form>
  ...
  <div class="card">
    ...
    <div class="log-item">
      ...
      <div>
        ...
        <?php if (!empty($entry['out'])): ?>
        <div class="detail"><?= htmlspecialchars($entry['out']) ?></div>
        <?php endif; ?>
      </div>
    </div>
    <?php endforeach; ?>
    ...
    <div class="btn-center">
      <a href="<?= htmlspecialchars($result['url'] ?? '/') ?>" class="btn btn-primary">&#x1F3E0; الذهاب إلى الموقع</a>
      <a href="<?= htmlspecialchars($result['url'] ?? '/') ?>/admin" class="btn btn-secondary" style="margin-right:.8rem">لوحة التحكم</a>
    </div>
    <?php else: ?>
    <div class="alert alert-error" style="margin-top:1.5rem">❌ فشل التنصيب — راجع الأخطاء أعلاه.</div>
    <div class="btn-center"><a href="?step=config" class="btn btn-secondary">العودة للإعدادات</a></div>
    <?php endif; ?>
  </div>

  <?php endif; ?>

  <p style="text-align:center;color:#475569;font-size:.75rem;margin-top:2rem">NIZARI Rachid Portfolio &copy; <?= date('Y') ?> — هذه الصفحة للاستخدام مرة واحدة فقط</p>
